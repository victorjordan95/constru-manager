...
describe('GET /fixed-expenses/:id', () => {
  ...
  beforeAll(async () => {
    const res = await request(app)
      .post('/fixed-expenses')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ name: `${UNIQUE} ToGet`, amount: 30000, dueDay: 7 });
    expenseId = res.body.id;
  });

  it('returns the expense by id (ADMIN)', async () => {
    const res = await request(app)
      .get(`/fixed-expenses/${expenseId}`)
      .set('Authorization', `Bearer ${adminToken}`);
    expect(res.status).toBe(200);
    expect(res.body.id).toBe(expenseId);
    expect(res.body.name).toBe(`${UNIQUE} ToGet`);
  });

  it('returns the expense by id (FINANCE)', async () => {
    const res = await request(app)
      .get(`/fixed-expenses/${expenseId}`)
      .set('Authorization', `Bearer ${financeToken}`);
    expect(res.status).toBe(200);
  ...
  it('returns 404 for unknown id', async () => {
    const res = await request(app)
      .get('/fixed-expenses/nonexistent')
      .set('Authorization', `Bearer ${adminToken}`);
