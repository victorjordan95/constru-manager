...
import type { FinanceSummary, CashflowMonth, OverdueInstallment, DREResponse } from './types';
...
export async function getDRE(month: number, year: number): Promise<DREResponse> {
  const { data } = await api.get<DREResponse>(`/finance/dre?month=${month}&year=${year}`);
  return data;
}
